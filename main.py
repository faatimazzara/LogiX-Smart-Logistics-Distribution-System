...
def tambah_barang():
    ...
    while True:
        ...
        if id_b not in db.data.get("barang", {}):
            break

        print(f"{Theme.FAIL}❌ ID sudah digunakan!{Theme.ENDC}")
        if input("Coba input lagi? (y/n): ").lower() != "y":
            print("❌ Input barang dibatalkan")
            return
    ...
    while True:
        ...
        if input("Mau coba input lagi? (y/n): ").lower() != "y":
            print("❌ Input barang dibatalkan")
            return
...
def tambah_toko():
    ...
    # KONFIRMASI AWAL
    while True:
        konfirmasi = input("Yakin ingin menambahkan toko? (y/n): ").lower()
...
def distribusi():
    ...
    if jumlah > stok_tersedia:
        print(f"❌ Stok tidak cukup! (Sisa: {stok_tersedia})")
        input("Enter...")
        return
